...
## How it works
- The section uses a `radial-gradient` where the center is positioned using `var(--ease-cursor-x)` and `var(--ease-cursor-y)`, which default to `50% 50%`.
- A `mousemove` (and `touchmove`) event listener calculates the cursor’s position relative to the section and updates the custom properties.
- A short CSS `transition` on the `background` property creates a smooth, lagging follow effect.
